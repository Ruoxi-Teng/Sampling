...
for num_clinics in [5, 10, 20]:
    min_cost_results = pd.read_csv(f"Data/Min Cost {num_clinics} sites selected_Simulated.csv")
    plot = plot_min_cost_customized_results(min_cost_results,treatment_stats_sum,treatment_stats_customized,num_clinics)
    plot.savefig(f'Figures/Min Cost {num_clinics} sites Customized_Simulated.png')
    plt.show()
...
results_dict = {}
for num_clinics in [5, 10, 20]:
    results_dict[num_clinics] = pd.read_csv(f"Data/Min Cost {num_clinics} sites selected_Simulated.csv")

# Plot and save
plot = plot_min_cost_combined_results(results_dict, treatment_stats_sum, treatment_stats_customized)
plot.savefig('Figures/Combined_Min_Cost_Sites_Customized_Simulated.png')
plt.show()
